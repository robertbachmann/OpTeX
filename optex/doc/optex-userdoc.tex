...
`optex` in your \TeX/ distribution. Then you can type into the command line:
...
The first line \~`\fontfam[LMfonts]` loads the Unicode Latin Modern\fnote{
    An extended version of Computer Modern.}
fonts are used. If you omit this line then
...
A somewhat larger example with common settings would be:
...
The \^`\margins` command declares the document's margins.
...
\begitems
* <pg> \dots\ `1` or `2` specifies one-page or two-pages (two-sided/duplex) design.
* <fmt> \dots\ paper format (a4, a4l, a5, letter, etc. or user defined).
* <left>, <right>, <top>, <bot> \dots\ gives the amount of left, right,
      top and bottom margins.
* <unit> \dots\ unit used for values <left>, <right>, <top>, <bottom>.
\enditems

Each of the parameters <left>, <right>, <top>, <bottom> can be empty.
If both <left> and <right> are non-empty then `\hsize` is set, otherwise `\hsize`
remains unchanged. If both <left> and <right> are empty then the typesetting area is
centered in the paper format. The analogical rule applies when <top> or <bot>
...
declared margins apply to odd pages. The margins for even pages are
automatically mirrored: <left> is replaced by
...
optional. If it is missing then `<unit>` after the margin specification is
used. For example:
...
before and after the <fmt> parameter are mandatory.
...
and pages remain unchanged. This can be useful for
...
is numbered
\new
by chapter starting from one\fnote
...
given by the position of \^`\mnote` in the text. Exceptions are possible
...
When a font family is loaded then {\em font modifiers} applicable in such font family
...
The font modifiers such as `\caps` or `\cond` can
...
* \bgroup\it This is {\em important} text.\egroup
* \bgroup\bf This is {\em important} text.\egroup
* \bgroup\bi This is {\em important} text.\egroup
\enditems
...
You can mix multiple font families in your document, and you can
declare your own variant selectors and modifiers.
